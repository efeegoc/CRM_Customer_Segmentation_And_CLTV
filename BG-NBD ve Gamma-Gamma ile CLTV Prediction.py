...
# OmniChannel.csv verisini okuyalım.Dataframe’in kopyasını oluşturalım.
df_ = pd.read_csv("datasets/flo_data_20K.csv")
df = df_.copy()
# CLTV (Customer Lifetime Value - Müşteri Yaşam Boyu Değeri) hesaplanırken verilerin doğru ve güvenilir olması büyük önem taşır.
# Aykırı değerler, hesaplamaların doğruluğunu bozabilir ve yanıltıcı sonuçlara neden olabilir.
# Bu nedenle, outlier_thresholds ve replace_with_thresholds fonksiyonları kullanılarak aykırı değerler belirlenir ve bu değerler belirlenen eşik değerleriyle değiştirilir.
...
# ve 6.aydaki en çok satın alım gerçekleştirecek 10 kişiyi inceleyelim. Fark var mı?
...
# 6 aylık standartlaştırılmış CLTV'ye göre tüm müşterilerimizi 4 gruba (segmente) ayıralım ve grup isimlerini veri setine ekleyelim.
# cltv_segment ismi ile atayalım.
cltv_df["cltv_segment"] = pd.qcut(cltv_df["cltv"], 4, labels=["D", "C", "B", "A"])
cltv_df.head(20)

# CLTV skorlarına göre müşterileri 4 gruba ayırmak mantıklı mıdır? Daha az mı ya da daha çok mu olmalıdır. Yorumlayalım.
# Örnek olarak A segmentinde fark varmış gibi görünüyor . Duruma göre A segmenti gibi yada diğer segmentler de incelenerek
# sayısı arttırılabilir.

# 4 grup içerisinden seçeceğimiz 2 grup için yönetime kısa kısa 6 aylık aksiyon önerilerinde bulalım
# Örnek olarak A segment çok değerli olduğu için onlara yönelik teşvik edici hediyeler edilebilir
# numuneler gönderilebilir seminer düzenlenebilir.
# B segmenti içinde örnek olarak sms gönderilebilir
# Aynı şekilde diğer segmentlerdede farklı opsiyonlar değerlendirilebilir.
